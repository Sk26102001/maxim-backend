...
import InventoryLog2 from "../../models/vendor/inventoryLog.model.js"
...
export const createLog = async (req, res) => {
    try {
        ...
        // 🔹 Save log in DB
        const savedLog = await InventoryLog2.create(logEntry)
...
export const getLogById = async (req, res) => {
    try {
        ...
        const log = await InventoryLog2.findById(logId)
...
export const getAllInventoryLogs = async (req, res) => {
    try {
        const logs = await InventoryLog2.find().sort({ createdAt: -1 }) // latest first
