...
import { cleanupTempFiles, generateETicketPDF, generateInvoicePDF } from "./pdfGenerator.js"
...
// Remove the old transporter declaration at the top
// let transporter = null  <-- REMOVE THIS LINE
...
//         // Get the latest email settings from database
//         emailSettings = await EmailSetting.findOne().sort({ createdAt: -1 })

//         if (!emailSettings) {
//             throw new Error("No email settings found in database. Please configure email settings first.")
//         }
...
// }
const getEmailSettings = async () => {
    const latestSettings = await EmailSetting.findOne().sort({ createdAt: -1 })

    if (!latestSettings) {
        throw new Error("No email settings found in database.")
    }

    // If no cache yet → set it
    if (!emailSettings) {
        emailSettings = latestSettings
        return emailSettings
    }

    // If credentials changed → refresh transporter
    if (emailSettings.user !== latestSettings.user || emailSettings.password !== latestSettings.password) {
        console.log("🔄 Email credentials changed. Refreshing transporter...")

        emailSettings = latestSettings
        transporterInstance = null // force reinitialize
    }
...
/* ===============================
   INITIALIZE OR REFRESH TRANSPORTER
================================ */
let transporterInstance = null // <-- USE A DIFFERENT NAME FOR THE INTERNAL TRANSPORTER
...
const initializeTransporter = async () => {
    try {
        ...
        console.log("✅ Email transporter initialized with database settings")
...
/* ===============================
   GET TRANSPORTER (WITH INITIALIZATION IF NEEDED)
================================ */
...
const refreshEmailSettings = async () => {
    try {
        ...
        // Reinitialize
        await initializeTransporter()
        console.log("✅ Email settings refreshed successfully")
        return true
    } catch (error) {
        console.error("❌ Failed to refresh email settings:", error.message)
        throw error
    }
}
...
export const sendPassengerEmail = async ({ passenger, inventory, inventoryDate }) => {
    ...
    try {
        ...
        // Get email settings and transporter
...
Safe Travels!
        ...
        const mailOptions = {
            from: `"${settings.emailName || "Flight Booking"}" <${settings.emailFrom}>`,
            to: passenger.email,
        ...
        // If error is related to authentication, try to refresh settings
        if (error.code === "EAUTH" || error.message.includes("authentication")) {
            console.log("🔄 Authentication error detected, refreshing email settings...")
...
export const sendInvoiceEmail = async ({ passenger, inventory, inventoryDate }) => {
    ...
    try {
        ...
        // const totalAmount = passengerPrice + infantPrice
        // Check if ALL 5 infant fields are present
        const hasValidInfant =
            passenger.infant &&
            passenger.infant.title &&
            passenger.infant.firstName &&
            passenger.infant.lastName &&
            passenger.infant.dob &&
            passenger.infant.price !== undefined &&
            passenger.infant.price !== null

        const infantPrice = hasValidInfant ? Number(passenger.infant.price) : 0
...
${hasValidInfant  ? `Infant Charge  1    ₹${infantPrice.toLocaleString("en-IN")}  ₹${infantPrice.toLocaleString("en-IN")}` : ""}
===========================================
...
${process.env.COMPANY_NAME || "Travel Agency"}
        ...
        const mailOptions = {
            from: `"${settings.emailName || "Invoice Department"}" <${settings.emailFrom}>`,
            to: passenger.email,
...
// For backward compatibility, export a transporter object that lazily initializes
...
// // import path from "path"
// // import { fileURLToPath } from "url"
// // import EmailSetting from "../models/EmailSetting.js" // Adjust path as needed
// // import { cleanupTempFiles, generateETicketPDF, generateInvoicePDF } from "./pdfGenerator.js"
...
// //    REFRESH EMAIL SETTINGS
// // ================================ */
// // export const refreshEmailSettings = async () => {
// //     try {
// //         // Clear cache
// //         emailSettings = null
// //         transporter = null
...
// // //                     <div class="value" style="font-weight: bold;">₹${passenger.price.toLocaleString("en-IN")}</div>
// // //                 </div>
// // //                 `
// // //                         : ""
// // //                 }
...
// //         // Determine if infant is present
// //         const hasInfant = passenger.infant && (passenger.infant.firstName || passenger.infant.price)
// //         const infantName = hasInfant ? `${passenger.infant.firstName || "Infant"}` : null
...
// // PASSENGER DETAILS:
// // - Name: ${passenger.title} ${passenger.firstName} ${passenger.lastName || ""}
// // ${hasInfant ? `- Infant Name: ${infantName}\n` : ""}
// // - Reference ID: ${passenger.refId || "N/A"}
// // - Seat Number: ${passenger.seat || "-"}
// // - Status: ${passenger.status?.toUpperCase() || "CONFIRMED"}
...
// //         ? `
// // PAYMENT SUMMARY:
// // ${passengerPrice > 0 ? `- Passenger Fare: ₹${passengerPrice.toLocaleString("en-IN")}\n` : ""}
// // ${infantPrice > 0 ? `- Infant Charge: ₹${infantPrice.toLocaleString("en-IN")}\n` : ""}
// // - Total Amount: ₹${totalAmount.toLocaleString("en-IN")}
// // `
// //         : ""
// // }
...
// // IMPORTANT: Please carry a valid government-issued photo ID to the airport along with your e-ticket.
// // ${hasInfant ? `\nINFANT TRAVEL NOTICE: Please carry the infant's birth certificate or any valid age proof document for verification.` : ""}
...
// // // For backward compatibility, export a dummy transporter
// // // Note: This won't be initialized until getTransporter() is called
// // export { transporter }
