import Inventory2 from "../../models/vendor/inventory.model.js"
import Series from "../../models/vendor/series.model.js"
...
export const addInventory = async (req, res) => {
    try {
        ...
        const sanitizedInventoryDates = inventoryDates.map((item) => ({
            ...
            // ✅ AIRLINE SNAPSHOT - INCLUDING fareId
            airlineName: series.airlineName,
            airlineCode: series.airlineCode,
            airlineNumber: series.airlineNumber,
            fareClass: series.fareClass,
            ...
            departTime: series.departTime,
            arriveTime: series.arriveTime,
            fareId: series.fareId,  // ✅ ADD THIS LINE
        }))

        const inventory = await Inventory2.create({
            seriesId,
            agencyId,
            fareValidity,
            dateFrom: new Date(dateFrom),
...
export const getInventoryDateList = async (req, res) => {
    try {
        const inventoryDates = await Inventory2.aggregate([
...
//     try {
//         const { id } = req.params
//         const inventory = await Inventory.findById(id).populate("seriesId")
...
export const getInventoryDates = async (req, res) => {
    try {
        const { id } = req.params;
        console.log("Received Series ID:", id);

        // Find inventory by seriesId (not by _id)
        const inventory = await Inventory2.findOne({ seriesId: id }).populate("seriesId");
        ...
        if (!inventory) {
            return res.status(404).json({
                status: "fail",
                message: "No inventory found for this series",
            });
        }
        ...
        console.error("ERROR in getInventoryDates:", error);
        res.status(500).json({
            status: "error",
            message: "Server error",
        });
...
export const deleteSeries = async (req, res) => {
    try {
        ...
        await Inventory2.deleteMany({ seriesId: id })
...
export const editInventory = async (req, res) => {
    try {
        ...
        const inventory = await Inventory2.findById(id)
        if (!inventory) {
...
export const getInventoryById = async (req, res) => {
    try {
        const inventory = await Inventory2.findById(req.params.id).populate("seriesId")
...
export const getAllInventories = async (req, res) => {
    try {
        const inventories = await Inventory2.find().populate("seriesId").sort({ createdAt: -1 })
...
//         const data = await Inventory2.aggregate([
//             { $unwind: "$inventoryDates" },
...
//                     // ✅ ALWAYS GET FAREID FROM SERIES
//                     "inventoryDates.fareId": "$series.fareId",
//                 },
//             },
...
//                     // ✅ SERIES REFERENCE AND STATUS
//                     seriesId: "$series._id",
//                     // fareId: "$inventoryDates.fareId",  // Get from inventoryDates where we added it
//                     fareId: "$series.fareId",
...
//         })
//     } catch (error) {
//         console.error("Error in getUpcomingInventoryDates:", error)
//         return res.status(500).json({
//             status: "error",
//             message: "Server error",
//         })
...
export const getUpcomingInventoryDates = async (req, res) => {
    try {
        ...
        const data = await Inventory2.aggregate([
            ...
            {
                $lookup: {
                    from: "series2", // Try "series2" instead of "series"
                    localField: "seriesId",
                    foreignField: "_id",
                    as: "series",
                },
            },
            { $unwind: { path: "$series", preserveNullAndEmptyArrays: false } }, // Set to false to exclude documents without series

            {
                $addFields: {
                    ...
                    "inventoryDates.fareId": {
   $ifNull: ["$inventoryDates.fareId", "$series.fareId"]
}
                },
            },

            {
                $project: {
                    ...
                    // ✅ Get fareId directly from series
                    seriesId: "$series._id",
                    // fareId: "$series.fareId",
                    fareId: "$inventoryDates.fareId",
                    seriesStatus: "$series.status",
...
//                     seriesStatus: "$series.status",
//                     fareId: "$series.fareId",  // ✅ KEEP THIS ONE ONLY

//                     // ✅ Use enhanced inventoryDates fields
//                     airlineName: "$inventoryDates.airlineName",
//                     airlineCode: "$inventoryDates.airlineCode",
//                     airlineNumber: "$inventoryDates.airlineNumber",
//                     fareClass: "$inventoryDates.fareClass",
...
//         })
//     } catch (error) {
//         console.error("Error in getCompletedInventoryDates:", error)
//         return res.status(500).json({
//             status: "error",
//             message: "Server error",
//             error: error.message
...
export const getCompletedInventoryDates = async (req, res) => {
    try {
        ...
        const data = await Inventory2.aggregate([
            ...
            // ✅ IMPORTANT: exclude records without matching series
            { $unwind: { path: "$series", preserveNullAndEmptyArrays: false } },

            // ✅ Same enhancement logic as upcoming
            {
                $addFields: {
                    "inventoryDates.airlineName": {
                        $ifNull: ["$inventoryDates.airlineName", "$series.airlineName"],
...
export const updatePNRStatus = async (req, res) => {
    try {
        ...
        const inventory = await Inventory2.findOneAndUpdate(
...
export const editSingleInventoryDate = async (req, res) => {
    try {
        ...
        const inventory = await Inventory2.findOne({ "inventoryDates._id": inventoryDateId })
        if (!inventory) {
